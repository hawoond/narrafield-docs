...
사용법은 공개 데모를 기준으로 작성합니다. `content_status: planned` 문서는 개발 중인 기능과 시안을 소개합니다. `_data/review.yml`에 검토일·기획 버전·검증한 데모 버전을 기록합니다. 모든 가이드는 한국어와 영어로 제공하고 언어 탭으로 같은 문서를 전환합니다.
...
항구 일러스트는 콘셉트 아트입니다. 이전 개발 알파의 실제 앱 화면과 최신 제작기·플레이 화면 시안을 캡션으로 구분합니다. 시안에 나온 기능이 현재 데모에 모두 포함되어 있는 것은 아닙니다.
