...
Import backgrounds and portraits through the editor. Successfully imported images are included in exported games. Online access requires authentication and is limited to the current scene for players.
...
Edit items, quests, attributes, and initial inventory in **Game data → Items & quests**. Remove references before deleting definitions that scenes or expressions use. See [Rules and expressions](RULES.md) for supported operations.
...
- [Rules and expressions](RULES.md): define conditions, expressions, and game data.
...
We are working on ways to carry promises, world laws, and earlier choices into later scenes. Creators will also be able to distinguish established facts from rumors. See [A world that remembers](WORLD_STORIES.md) for the planned features.
