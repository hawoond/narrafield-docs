...
1. Write a detail about a place or character: perhaps lighting a lighthouse requires someone's memory.
2. Connect it to an action's conditions, costs, and effects. Descriptive text alone does not execute a rule.
3. Define the cost and another route when the player declines it.
...
These configurations are planned inside the [single starter-template catalog](STARTER_CONTENT.md). Story content, a bundle of play settings, and the amount of editing guidance are separate choices, using the same project format.

| Configuration | Starting and playing | Settings to coordinate |
| --- | --- | --- |
| Solo narrative | Conditions and choices without a GM; no character required by default | Local saves, text or scene-focused screens, optional characters |
| GM-led cooperative | A GM and players start with prepared character candidates | Shared travel, proposals and affected players' consent, server saves, RP/OOC/whispers |
| Automated party | Registered rules without a GM; one character per player by default | One vote per player, canceled proposals and reselection on ties/timeouts, safe absence handling |
...
Follow the [Quick start](QUICKSTART.md) for the public demo's actual menus and samples. The complete three-configuration workflow and new samples are development goals. [Development stages](DEVELOPMENT.md) distinguish the first-release goals from later expansion.

[Choices the world remembers](WORLD_STORIES.md) · [Starter content](STARTER_CONTENT.md) · [Testing and release](TEST_AND_RELEASE.md)
